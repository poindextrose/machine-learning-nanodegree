...
class LearningAgent(Agent):
    ...
    def __max_action_value(self, state):
        """Find the action with the highest Q-value and return the action-Q_value pair"""
        return max(self.Q[state].iteritems(), key=operator.itemgetter(1))

    def get_maxQ(self, state):
        """ The get_max_Q function is called when the agent is asked to find the
            maximum Q-value of all actions based on the 'state' the smartcab is in. """
        return self.__max_action_value(state)[1]
    ...
    def choose_action(self, state):
        ...
        if random.random() < self.epsilon:
            ...
            action = self.__max_action_value(state)[0]
        return action
